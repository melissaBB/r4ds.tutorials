...
In this chapter, you'll delve into the world of iteration in R, where you repeatedly apply the same action to various objects. Unlike many other programming languages, R often handles iteration implicitly, providing a more intuitive and concise approach. 

We have already introduced you to some powerful tools for performing actions across multiple elements, such as `facet_wrap()` and `facet_grid()` for plotting subsets, `group_by()` and `summarize()` for computing summary statistics, and `unnest_wider()` and `unnest_longer()` for expanding list-columns into rows and columns.

Now, you'll explore more general tools, often referred to as **functional programming** tools, which revolve around functions that take other functions as inputs. We will keep things practical by focusing on three common tasks: **modifying multiple columns, reading multiple files, and saving multiple objects.**
...
In this section, you'll learn how to use `purrr::map()` to do something to every file in a directory.You will also learn to use `list_rbind()` to create one data table from a list of many data table or data frame objects. Simple iterations will also be introduced here. 
...
```{r reading-multiple-fil-23-hint-1, eval = FALSE}
paths |> 
  set_names(...) |> 
  ...(readxl::read_excel) |> 
  list_rbind(...) |> 
...
```{r reading-multiple-fil-46-hint-1, eval = FALSE}
... <- paths |> 
  map(possibly(\(path) readxl::read_excel(...), NULL))
```
...
```{r reading-multiple-fil-47, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r reading-multiple-fil-47-hint-1, eval = FALSE}
... |> list_rbind()
```
...
```{r reading-multiple-fil-48, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r reading-multiple-fil-48-hint-1, eval = FALSE}
map_vec(..., is.null)
```
...
```{r reading-multiple-fil-49, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r reading-multiple-fil-49-hint-1, eval = FALSE}
failed <- ...(files, is.null)
```
...
```{r reading-multiple-fil-50, exercise = TRUE}
paths[failed]
```
...
In this sub-chapter, you will explore ways to cfreate a a data base, csv files, and how to save a plot. 
...
```{r saving-multiple-outp-23-hint, eval = FALSE}
function(df) {}
```
...
```{r saving-multiple-outp-24-hint, eval = FALSE}
function(df) {
  ggplot()}
```
...
```{r saving-multiple-outp-25-hint, eval = FALSE}
function(...) {
...
```{r saving-multiple-outp-26-hint, eval = FALSE}
function(df) {
...
```{r saving-multiple-outp-27-hint, eval = FALSE}
function(df) {
  ...(df, aes(x = carat)) + ...(binwidth = 0.1)}
```
...
```{r saving-multiple-outp-28-hint, eval = FALSE}
carat_histogram <- function(df) {
  ggplot(...) + geom_histogram(...)}
```
...
```{r saving-multiple-outp-29-hint, eval = FALSE}
by_clarity |> 
  ...(
    plot = map(...),
    ... = str_glue("clarity-{clarity}.png")
  )
```
...
```{r saving-multiple-outp-30-hint, eval = FALSE}
by_clarity <- by_clarity |> 
  mutate(
    plot = ...,
    path = ...("clarity-{clarity}.png")
  )
```
...
```{r saving-multiple-outp-31-hint, eval = FALSE}
walk2(
  ...,
  by_clarity$plot)
```
...
```{r saving-multiple-outp-32-hint, eval = FALSE}
walk2(
  ...,
  by_clarity$plot,
  \(path, plot) ggsave())
...
```{r saving-multiple-outp-33-hint, eval = FALSE}
walk2(
  ...,
  by_clarity$plot,
